...
<script lang="ts" setup>
  ...
  async function handlerUpateViolationStatus() {
    const res: any = await ApiUser.updateUserStatus(form);
    res.success && ElMessage({ type: 'success', message: '变更记录状态成功！' });
    visible.value = false;
    queryAllLog();
  }
  ...
  <div>
      ...
      >
        ...
        <el-table-column prop="status" label="用户状态" width="120" align="center">
          <template #default="{ row }: { row: any }">
            <el-tag :type="USER_STATUS_TYPE_MAP[row.userInfo.status as UserStatus]">
    ...
    <el-dialog v-model="visible" title="变更用户状态" width="500px">
      <el-form :model="form" :inline="true">
        ...
        <el-form-item>
          <el-button type="primary" @click="handlerUpateViolationStatus"> 确认变更 </el-button>
        </el-form-item>
      </el-form>
    </el-dialog>
  </div>
