...
<script lang="ts" setup>
  ...
  const rules = reactive<FormRules>({
    ...
    drawMjCount: [{ required: true, message: '卡密携带绘画数量', trigger: 'blur' }],
    model3Count: [{ required: true, message: '卡密携带基础模型对话数量', trigger: 'blur' }],
    model4Count: [{ required: true, message: '卡密携带高级模型金额', trigger: 'blur' }],
  });
  ...
  <div>
    <PageHeader>
      ...
      <template #content>
        <div class="text-sm/6">
          ...
          <div>过期时间表示卡密的过期时间，不是用户充值后的有效期，设置为0表示永不过期。</div>
        </div>
      </template>

      <HButton outline v-if="selects.length" type="danger" @click="handleShowCrami">
        显示选中卡密
      </HButton>
      <HButton outline v-if="selects.length" type="danger" @click="batchDelCrami">
        批量删除卡密
      </HButton>
      <HButton outline v-if="selects.length" type="primary" @click="exportCrami">
        批量导出卡密
      </HButton>
    ...
    <page-main>
      <el-form ref="formRef" :inline="true" :model="formInline" class="mb-4">
        ...
        <el-form-item>
          <el-button type="primary" @click="queryAllCramiList"> 查询 </el-button>
    ...
    <el-dialog v-model="visible" title="生成卡密" width="450" @close="handleClose(formCramiRef)">
      ...
      >
        ...
        <el-form-item label="生成数量" prop="count">
          <el-input v-model.number="form.count" type="number" placeholder="本次生成的张数" />
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="visible = false">放弃生成</el-button>
          <el-button type="primary" @click="handlerCreateCrami(formCramiRef)"> 确定生成 </el-button>
        </span>
      </template>
    </el-dialog>
